-- GlassOS RLS — corrected for 57-table schema
...
-- === SECTION 2: Tables WITHOUT tenant_id — EXISTS through parent ===
...
CREATE POLICY tenant_isolation_customer_contacts ON customer_contacts
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM customers WHERE customers.id = customer_contacts.customer_id AND customers.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM customers WHERE customers.id = customer_contacts.customer_id AND customers.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_customer_delivery_points ON customer_delivery_points
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM customers WHERE customers.id = customer_delivery_points.customer_id AND customers.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM customers WHERE customers.id = customer_delivery_points.customer_id AND customers.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_cutting_result_items ON cutting_result_items
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM cutting_results WHERE cutting_results.id = cutting_result_items.cutting_result_id AND cutting_results.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM cutting_results WHERE cutting_results.id = cutting_result_items.cutting_result_id AND cutting_results.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_emergency_contacts ON emergency_contacts
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = emergency_contacts.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = emergency_contacts.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_grinding_profiles ON grinding_profiles
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM factories WHERE factories.id = grinding_profiles.factory_id AND factories.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM factories WHERE factories.id = grinding_profiles.factory_id AND factories.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_inventory_barcodes ON inventory_barcodes
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM inventory_items WHERE inventory_items.id = inventory_barcodes.inventory_item_id AND inventory_items.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM inventory_items WHERE inventory_items.id = inventory_barcodes.inventory_item_id AND inventory_items.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_inventory_lots ON inventory_lots
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM inventory_items WHERE inventory_items.id = inventory_lots.inventory_item_id AND inventory_items.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM inventory_items WHERE inventory_items.id = inventory_lots.inventory_item_id AND inventory_items.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_machine_consumables ON machine_consumables
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM machines WHERE machines.id = machine_consumables.machine_id AND machines.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM machines WHERE machines.id = machine_consumables.machine_id AND machines.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_machine_maintenance_logs ON machine_maintenance_logs
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM machines WHERE machines.id = machine_maintenance_logs.machine_id AND machines.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM machines WHERE machines.id = machine_maintenance_logs.machine_id AND machines.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_machine_spare_parts ON machine_spare_parts
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM machines WHERE machines.id = machine_spare_parts.machine_id AND machines.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM machines WHERE machines.id = machine_spare_parts.machine_id AND machines.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_material_unit_profiles ON material_unit_profiles
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM materials WHERE materials.id = material_unit_profiles.material_id AND materials.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM materials WHERE materials.id = material_unit_profiles.material_id AND materials.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_order_lines ON order_lines
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM orders WHERE orders.id = order_lines.order_id AND orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM orders WHERE orders.id = order_lines.order_id AND orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_order_notes ON order_notes
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM orders WHERE orders.id = order_notes.order_id AND orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM orders WHERE orders.id = order_notes.order_id AND orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_personnel_certificates ON personnel_certificates
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_certificates.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_certificates.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_personnel_health_information ON personnel_health_information
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_health_information.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_health_information.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_personnel_machine_assignments ON personnel_machine_assignments
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_machine_assignments.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_machine_assignments.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_personnel_shifts ON personnel_shifts
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_shifts.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_shifts.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_personnel_station_permissions ON personnel_station_permissions
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_station_permissions.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM personnel WHERE personnel.id = personnel_station_permissions.personnel_id AND personnel.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_production_breakage_events ON production_breakage_events
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM production_orders WHERE production_orders.id = production_breakage_events.production_order_id AND production_orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM production_orders WHERE production_orders.id = production_breakage_events.production_order_id AND production_orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_production_events ON production_events
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM production_orders WHERE production_orders.id = production_events.production_order_id AND production_orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM production_orders WHERE production_orders.id = production_events.production_order_id AND production_orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_production_queue_items ON production_queue_items
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM production_queues WHERE production_queues.id = production_queue_items.queue_id AND production_queues.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM production_queues WHERE production_queues.id = production_queue_items.queue_id AND production_queues.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_recipe_items ON recipe_items
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM recipes WHERE recipes.id = recipe_items.recipe_id AND recipes.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM recipes WHERE recipes.id = recipe_items.recipe_id AND recipes.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_recipe_operations ON recipe_operations
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM recipes WHERE recipes.id = recipe_operations.recipe_id AND recipes.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM recipes WHERE recipes.id = recipe_operations.recipe_id AND recipes.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_recipe_rules ON recipe_rules
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM recipes WHERE recipes.id = recipe_rules.recipe_id AND recipes.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM recipes WHERE recipes.id = recipe_rules.recipe_id AND recipes.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_recipe_versions ON recipe_versions
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM recipes WHERE recipes.id = recipe_versions.recipe_id AND recipes.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM recipes WHERE recipes.id = recipe_versions.recipe_id AND recipes.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_remnant_thresholds ON remnant_thresholds
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM factories WHERE factories.id = remnant_thresholds.factory_id AND factories.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM factories WHERE factories.id = remnant_thresholds.factory_id AND factories.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_rework_history ON rework_history
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM rework_orders WHERE rework_orders.id = rework_history.rework_order_id AND rework_orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM rework_orders WHERE rework_orders.id = rework_history.rework_order_id AND rework_orders.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_station_machine_assignments ON station_machine_assignments
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM stations WHERE stations.id = station_machine_assignments.station_id AND stations.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM stations WHERE stations.id = station_machine_assignments.station_id AND stations.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_station_personnel_assignments ON station_personnel_assignments
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM stations WHERE stations.id = station_personnel_assignments.station_id AND stations.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM stations WHERE stations.id = station_personnel_assignments.station_id AND stations.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_trim_profiles ON trim_profiles
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM factories WHERE factories.id = trim_profiles.factory_id AND factories.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM factories WHERE factories.id = trim_profiles.factory_id AND factories.tenant_id = current_setting('app.current_tenant_id', true)::char(26))
  );
...
CREATE POLICY tenant_isolation_user_sessions ON user_sessions
  FOR ALL
  USING (
    EXISTS (SELECT 1 FROM users WHERE users.id = user_sessions.user_id AND (users.tenant_id = current_setting('app.current_tenant_id', true)::char(26) OR users.tenant_id IS NULL))
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM users WHERE users.id = user_sessions.user_id AND (users.tenant_id = current_setting('app.current_tenant_id', true)::char(26) OR users.tenant_id IS NULL))
  );
