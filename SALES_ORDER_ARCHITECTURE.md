...
> **Versiyon:** 2.0 — Tüm düzeltmeler uygulandı, onay bekliyor  
> **Tarih:** 2026-07-24  
> **Değişiklikler:** recipe_id kaldırıldı, JSONB alanlar kaldırıldı, features kaldırıldı. Snapshot alanları eklendi (thickness, glassColor, glassType, lineArea, totalArea). closed durumu eklendi. Üretim/sevkiyat durumları kapsam dışına çıkarıldı.
...
> **Not:** Üretim Planlama (`ProductionPlanner`) ve Sevkiyat (`Shipping`) rolleri Sipariş Modülü'nün kapsamı dışındadır. Bu roller sırasıyla **Kesim Planı Modülü** ve **Sevkiyat Modülü** altında tanımlanacaktır.

### 3.2. Hangi Ekranı Kim Kullanır?

| Ekran | Satış | Satış Müd. | Üretim Müd. | Yönetici |
|-------|:-----:|:----------:|:-----------:|:--------:|
| `/orders` (liste) | ✅ Kendi | ✅ Tümü | ✅ Tümü | ✅ Tümü |
| `/orders/new` | ✅ | ✅ | ❌ | ✅ |
| `/orders/[id]` (detay) | ✅ | ✅ | ✅ | ✅ |
| `/orders/[id]` (onayla) | ❌ | ✅ | ❌ | ✅ |
...
```mermaid
flowchart LR
    A[Taslak] -->|Onayla| B[Onaylandı]
    B -->|Üretim başlar| C[Üretimde]
    C -->|Tüm kalemler tamamlanır| D[Tamamlandı]
    D -->|Kapat| E[Kapalı]
...
**Kullanıcı:** Satış Temsilcisi, Satış Müdürü, Üretim Müdürü, Yönetici
...
│                                                         │
│  ┌──────┬──────┬──────┬──────┬──────┐                  │
│  │Taslak│Onaylı│Üretim│Tmm   │Kapalı│  ← KPI Mini Bar   │
│  │  3   │  12  │  15  │  5   │  2   │                  │
│  └──────┴──────┴──────┴──────┴──────┘                  │
│                                                         │
│  ┌───────────────────────────────────────────────────┐  │
...
| Tamamlandı | Green | Sayı, toplam m² |
| Kapalı | Dark gray | Sayı, toplam m² |
...
| Açıklama | Input (text) | ❌ | Kaleme özel not (örn. "Baca", "Menfez") |
...
|-------------|---------------|-----------|-------|
| Taslak (draft) | Onayla | Onaylandı | SalesManager / Admin |
| Taslak (draft) | Düzenle | (form) | Sales / Admin |
| Taslak (draft) | Sil | (soft delete) | Sales / Admin |
| Onaylandı (confirmed) | İptal | İptal | ProductionManager / Admin |
| Tamamlandı (completed) | Kapat | Kapalı | SalesManager / Admin |
...
┌─── Sipariş Kalemleri ──────────────────────────────────┐
│                                                        │
│  # │Ürün          │ En    │ Boy   │ Adet │ Durum     │
│  ──│──────────────│───────│───────│─────│──────────│
│  1 │8mm Füme Temp │ 1200  │ 2000  │ 15  │⏳ Üretimde│
│  2 │8mm Füme Temp │  800  │ 1500  │ 10  │✅ Tamam   │
│  3 │6+12+6 Isıcam │ 1000  │ 2200  │  5  │⏳ Üretimde│
│                                                        │
...
│  13.07 09:00  ● Üretimde — Üretim emri oluşturuldu    │
│                                                        │
└─────────────────────────────────────────────────────────┘
```
...
CREATE INDEX idx_orders_due_date ON orders(due_date) WHERE status NOT IN ('closed', 'cancelled');
...
| `CustomerRepository` | Müşteri doğrulama |
...
1. `OrderService` genişlet: yeni alanlar, snapshot alma, status geçişleri
